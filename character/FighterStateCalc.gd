...
const WALL_MAX_HIT_CALC: StringName = &"wall_max_hit_calc"
...
const INVINCIBLE_CALC: StringName = &"invincible_calc"
...
static func calc_max_wall_hit(me: Fighter) -> void:
	if me.states.has("actionable"):
		me.state_data.erase(INVINCIBLE_CALC)
		me.state_data.erase(WALL_MAX_HIT_CALC)
		return

	if me.states.has("wall_stun"):
		if !me.state_data.has(WALL_MAX_HIT_CALC):
			me.state_data.get_or_add(WALL_MAX_HIT_CALC, 0)
	else:
		if me.state_data.has(WALL_MAX_HIT_CALC):
			me.state_data[WALL_MAX_HIT_CALC] += 1
			if me.state_data[WALL_MAX_HIT_CALC] >= 4 && !me.states.has(INVINCIBLE_CALC):
				me.state_data.get_or_add(INVINCIBLE_CALC)
